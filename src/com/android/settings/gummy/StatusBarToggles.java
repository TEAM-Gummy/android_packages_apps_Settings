...
import com.android.internal.util.gummy.GummyConstants;
import com.android.internal.util.gummy.GummyConstants.GummyConstant;
import com.android.internal.util.gummy.GummyHelpers;
import com.android.internal.util.gummy.GummyConstants;
import com.android.settings.R;
import com.android.settings.SettingsPreferenceFragment;
import com.android.settings.gummy.objects.EasyPair;
import com.android.settings.util.ShortcutPickerHelper;
import com.android.settings.util.Helpers;
import com.android.settings.gummy.widgets.CustomTogglePref;
...
public class StatusBarToggles extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Get Toggle Actions
        mActionCodes = GummyHelpers.getGummyActions();
        mActions = new String[mActionCodes.length];
        int actionqty = mActions.length;
        for (int i = 0; i < actionqty; i++) {
            mActions[i] = GummyConstants.getProperName(mContext, mActionCodes[i]);
        }
    ...
    private Drawable getNavbarIconImage(String uri) {
        if (uri == null) {
            uri = GummyConstant.ACTION_NULL.value();
        }
        if (uri.startsWith("**")) {
    ...
    private String getProperSummary(String uri) {
        if (uri == null) {
            return GummyConstants.getProperName(mContext, "**null**");
        }
        if (uri.startsWith("**")) {
    ...
    private void updateSettings() {
        ...
        if (currentToggles != null) {
            ...
            if (mScreenshotDelay != null) {
                mScreenshotDelay
                        .setEnabled(currentToggles.contains("SCREENSHOT"));
            }
