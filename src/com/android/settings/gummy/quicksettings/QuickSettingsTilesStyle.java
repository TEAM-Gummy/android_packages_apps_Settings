...
public class QuickSettingsTilesStyle extends SettingsPreferenceFragment implements
    ...
    private PreferenceScreen refreshSettings() {
        ...
        mAdditionalOptions = (PreferenceCategory) prefs.findPreference(PREF_ADDITIONAL_OPTIONS);
        ...
        if (!DeviceUtils.isPhone(getActivity())) {
            mAdditionalOptions.removePreference(
                findPreference(PREF_TILES_PER_ROW_DUPLICATE_LANDSCAPE));
        }

        mQuickPulldown = (ListPreference) prefs.findPreference(QUICK_PULLDOWN);

        if (!DeviceUtils.isPhone(getActivity())) {
            if (mQuickPulldown != null) {
                mAdditionalOptions.removePreference(mQuickPulldown);
            }
        } else {
